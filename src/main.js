...
router.beforeEach((to, from, next) => {
    ...
    // 动态添加路由
    if (!options.length) {
        service({ url: '/get/router', method: 'get' }).then(res => {
            if (res.code === 200) {
                let data = rou.data
                store.commit('changeValue', { name: 'routers', value: data })
                data.forEach(item => {
...
function render({ container } = {}) {
    ...
    instance.use(Antd);
    instance.use(store).use(router).mount(container ? container.querySelector('#vue3-app') : '#vue3-app')
}
